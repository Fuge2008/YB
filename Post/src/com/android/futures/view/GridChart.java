...
//	public static final int DEFAULT_BORDER_COLOR = Color.RED;
	...
	protected void onDraw(Canvas canvas) {
		...
		latitudeSpacing = (viewHeight - 4 - DEFAULT_AXIS_TITLE_SIZE
				- topTitleHeight - mTabHight)
				/ (DEFAULT_UPER_LATITUDE_NUM + DEFAULT_LOWER_LATITUDE_NUM + 2);
		mUperChartHeight = latitudeSpacing * (DEFAULT_UPER_LATITUDE_NUM + 1);
		LOWER_CHART_TOP = viewHeight - 1 - latitudeSpacing
				* (DEFAULT_LOWER_LATITUDE_NUM + 1);
		UPER_CHART_BOTTOM = 1 + topTitleHeight + latitudeSpacing
	...
	private void drawRegions(Canvas canvas, int viewHeight, int viewWidth) {
		...
		canvas.drawLine(1, UPER_CHART_BOTTOM, viewWidth - 1, UPER_CHART_BOTTOM,
				paint);
		canvas.drawLine(1, LOWER_CHART_TOP, viewWidth - 1, LOWER_CHART_TOP,
				paint);
		if (showLowerChartTabs) {
			canvas.drawLine(1, UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE + 2,
					viewWidth - 1, UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE
							+ 2, paint);
			if (mLowerChartTabTitles == null
					|| mLowerChartTabTitles.length <= 0) {
				return;
			}
			mTabWidth = (viewWidth - 2) / 10.0f * 10.0f
					/ mLowerChartTabTitles.length;
			...
			for (int i = 0; i < mLowerChartTabTitles.length
					&& mTabWidth * (i + 1) <= viewWidth - 2; i++) {
				if (i == mTabIndex) {
					Paint bgPaint = new Paint();
					bgPaint.setColor(Color.MAGENTA);
					canvas.drawRect(mTabWidth * i + 1, LOWER_CHART_TOP,
							mTabWidth * (i + 1) + 1, UPER_CHART_BOTTOM
									+ DEFAULT_AXIS_TITLE_SIZE + 2, bgPaint);
				}
				canvas.drawLine(mTabWidth * i + 1, LOWER_CHART_TOP, mTabWidth
						* i + 1, UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE
						+ 2, paint);
//				canvas.drawText(mLowerChartTabTitles[i], mTabWidth * i
//						+ mTabWidth / 2.0f - mLowerChartTabTitles[i].length()
//						/ 3.0f * DEFAULT_AXIS_TITLE_SIZE, LOWER_CHART_TOP
//						- mTabHight / 2.0f + DEFAULT_AXIS_TITLE_SIZE / 2.0f,
//						textPaint);
			}
		}
	}
