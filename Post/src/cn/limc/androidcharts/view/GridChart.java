...
public class GridChart extends View implements IViewConst, ITouchEventNotify,
	...
	/** 默认经线刻度字体颜色 **/
	private int DEFAULT_LONGTITUDE_FONT_COLOR = Color.BLACK;

	/** 默认经线刻度字体颜色 **/
	private int DEFAULT_LONGTITUDE_FONT_SIZE = 15;
	...
	private void drawAlphaTextBox(PointF ptStart, PointF ptEnd, String content,
		...
		mPaintBox.setColor(Color.RED);
		// mPaintBox.setAlpha(90);
		...
		// 绘制矩形填�?
		canvas.drawRoundRect(new RectF(ptStart.x - 2, ptStart.y - 2,
				ptEnd.x + 2, ptEnd.y + 2), 0f, 0f, mPaintBox);

		// 绘制矩形�?
		// canvas.drawLine(ptStart.x, ptStart.y, ptStart.x, ptEnd.y,
		// mPaintBoxLine);
		...
		// canvas.drawLine(ptEnd.x, ptStart.y, ptStart.x, ptStart.y,
		// mPaintBoxLine);

		// 绘制�?��
		canvas.drawText(content, ptStart.x, ptEnd.y-2, mPaintBoxLine);
	}
	...
	protected void drawAxisGridX(Canvas canvas) {
		...
		if (null != axisXTitles) {
			...
			mPaintFont.setColor(getResources().getColor(R.color.black));
